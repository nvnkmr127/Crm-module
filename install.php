...
if (!$CI->db->table_exists(CUSTOM_ESTIMATION_TABLE_ESTIMATES)) {
    ...
    if (!$CI->db->field_exists('pdf_template_slug', CUSTOM_ESTIMATION_TABLE_ESTIMATES)) { 
        $CI->db->query("ALTER TABLE `" . CUSTOM_ESTIMATION_TABLE_ESTIMATES . "` ADD `pdf_template_slug` VARCHAR(100) NULL AFTER `{$estimates_after_col}`;"); 
        // Check if index exists before adding
        $query_index_check_pdf_slug = $CI->db->query("SHOW INDEX FROM `" . CUSTOM_ESTIMATION_TABLE_ESTIMATES . "` WHERE Key_name = 'pdf_template_slug'");
        if ($query_index_check_pdf_slug->num_rows() == 0) {
